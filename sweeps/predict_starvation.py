...
def load_routing(path, W, tokens_per_rank, topk):
    """Real per-token routing from a <matrix_id>.routing.txt sidecar (trace
    family): returns (cnt, tok_experts) in deal_experts' format, but with the
    trace's ACTUAL token-overlap structure. Rows are sorted per token (only
    the expert set matters for geometry). The dealer closed form does not hold
    for this input."""
    with open(path) as f:
        hdr = f.readline().split()
        ntokens, ftopk, G = int(hdr[0]), int(hdr[1]), int(hdr[2])
        assert ftopk == topk, f"routing topk {ftopk} != --topk {topk}"
        assert ntokens == W * tokens_per_rank, (
            f"routing has {ntokens} tokens, expected {W}x{tokens_per_rank}"
        )
        rows = [tuple(sorted(int(x) for x in line.split())) for line in f]
    assert len(rows) == ntokens
    tok_experts = [rows[s * tokens_per_rank : (s + 1) * tokens_per_rank] for s in range(W)]
    cnt = []
    for s in range(W):
        counts = [0] * G
        for es in tok_experts[s]:
            for e in es:
                counts[e] += 1
        cnt.append(counts)
    return cnt, tok_experts
...
def main():
    ...
    ap.add_argument(
        "--routing-file",
        help="<matrix_id>.routing.txt (trace family): use the REAL per-token"
        " routing instead of the dealer; requires --matrix",
    )
    ...
    if args.routing_file:
        ...
        cnt, tok_experts = load_routing(args.routing_file, W, T, args.topk)
        for s in range(W):
            got = [
                sum(cnt[s][d * (args.G // W) + j] for j in range(args.G // W)) for d in range(W)
            ]
            assert got == chunks[s], f"routing does not realize the matrix at row {s}"
    else:
        cnt, tok_experts = deal_experts(chunks, T, args.topk, args.G)

    # closed-form cross-check of the dealt U against gen_matrix's law (dealer
    # only — the closed form is invalid for real routing)
