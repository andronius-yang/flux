...
#SBATCH -t 5
#SBATCH -J weak16_dwire_b1
#SBATCH -o /pscratch/sd/y/yufeid/workspace/andrewy/logs/slurm/%x-%j.out

set -euo pipefail
cd /global/u1/y/yufeid/workspace/changchen/andrewy/flux
source ./module.sh
echo "=== weak16_dwire_b1 job $SLURM_JOB_ID on $(hostname), $(date -u +%FT%TZ) ==="
python3 sweeps/sweep.py run \
    --spec sweeps/specs/weak16_dwire_b1_k2.yaml \
    --jobid="$SLURM_JOB_ID"
echo "=== weak16_dwire_b1 done, $(date -u +%FT%TZ) ==="
