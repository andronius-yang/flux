...
source ./module.sh
echo "=== weak32_nvshmem job $SLURM_JOB_ID on $(hostname), $(date -u +%FT%TZ) ==="
python3 sweeps/sweep.py run \
    --spec sweeps/specs/weak32_nvshmem_k2.yaml \
    --jobid="$SLURM_JOB_ID"
echo "=== weak32_nvshmem done, $(date -u +%FT%TZ) ==="
