...
## v3 addendum (2026-09-17): `comet_eplb` and `comet_v3anchor` rows

- `comet_eplb` (18 rows, {4,8,16}n x {K2,Qwen} x {1,4,16} MiB): arm
  `l01_allgather_dense_eplb`, branch pv3 @ 859af86 (the main tree's Sep-10
  libs via the worktree symlinks), capsules 20260917-071106 / -071304 (4n
  K2/Qwen), -071215 / -071417 (8n), -073316 / -073515 (16n); statistic
  `iter_max_median`, recomputed from each capsule's `metrics.csv`
  (`recorded_ms` = the same value at 2 decimals; no manual ledger).
- `comet_v3anchor` (18 rows): the `l01_allgather_dense` cells of the SAME
  capsules — the same-binary COMET reference for the new bar. Not drawn by
  default; `main_perf_v3_samebinary.*` feeds the COMET bar from it.
- Drift ledger, official `comet` row (2026-08-24 binary, capsules
  20260824-15xxxx) vs `comet_v3anchor`: 4n K2 -9.6/-5.0/+0.3%, 4n Qwen
  -4.1/-1.0/-4.7%, 8n K2 -27.1/-12.3/-1.0%, 8n Qwen -13.5/-13.1/-6.7%, 16n K2
  -46.7/-19.7/-3.7%, 16n Qwen -29.4/-26.2/-7.1% at 1/4/16 MiB (today's
  COMET is faster). The large-budget cells agree within the run band; the
  small-budget cells do not — the official COMET row predates the 8/29 comm
  canon and later fixes on the shared code paths.
