...
CONFIG = dict(
    ...
    # v3 (2026-09-17): which figure_src row_id feeds a system's bar. Default =
    # the system's own id. The one sanctioned alternative is the COMET bar on
    # its SAME-BINARY anchor (`comet_v3anchor`, the l01_allgather_dense cells
    # of the 2026-09-17 COMET+EPLB capsules) — the official `comet` row is the
    # 2026-08-24 binary and is 4-47% slower than the same arm today at small
    # budgets (figure_src.md v3 addendum). main() renders both variants.
    ROW_SOURCE={},
    ...
    LEGEND_NAMES={                            # system -> legend text
        ...
        "comet": "COMET", "comet_eplb": "COMET+EPLB", "OURS": "Ours",
    },
    LEGEND_NCOL=8,                            # 8 = single row (v3); 4 -> 4+4 rows
    ...
    #      v3 2026-09-17: +comet_eplb deep plum #7a3b7a between olive and steel
    #      blue, re-validated with the same six-checks math (Python twin):
    #      adjacent CVD worst 10.7 (deutan, vs Ours), normal-vision worst 17.2,
    #      band/chroma PASS; the 7 original hexes are untouched) ----
    COLORS={
        "fast_gemm": "#c0653f", "nvshmem_gemm": "#ddaa33",
        "moonep": "#2f7a4d", "eplb": "#44bb99", "epic": "#9d4a4a",
        "comet": "#999933", "comet_eplb": "#7a3b7a", "OURS": "#4878b0",
    },
    ...
    # ---- output ----
    OUTPUTS=[("main_perf_v3.pdf", {}), ("main_perf_v3.png", {"dpi": 300})],
    # v3 variant: COMET bar on its same-binary anchor row (see ROW_SOURCE)
    VARIANTS=[("main_perf_v3_samebinary", {"comet": "comet_v3anchor"})],
)
# =============================================================================
            ...
            for b in cfg["BUDGETS"]:
                ...
                for sysname in cfg["SYSTEMS"]:
                    if sysname == "OURS":
                        ...
                        row_id = cfg.get("ROW_SOURCE", {}).get(sysname, sysname)
                        key = (nodes, model, b, row_id)
...
def main():
    ...
    for name, kw in CONFIG["OUTPUTS"]:
        ...
        print("wrote", name)
    for stem, row_source in CONFIG.get("VARIANTS", []):
        cfg = dict(CONFIG, ROW_SOURCE=row_source)
        vfig = plot(load(cfg), cfg)
        for name, kw in CONFIG["OUTPUTS"]:
            ext = name[name.rfind("."):]
            vfig.savefig(os.path.join(HERE, stem + ext), **kw)
