...
Table 1). K3 is a *Latent* MoE: the dispatched/combined row is the latent
`z` (dim 3584), not the model hidden (7168), so harness `H=3584` IS the
wire-authentic width; `W↓`/`W↑` and the 2 always-on shared experts sit
outside the dispatch/combine bracket by architecture and are not modeled.
No captured K3 routing exists — K3-shape cells use the synthesized-empirical
pools (`model=Kimi-K3-synth` in the trace family; generated by
`sweeps/synthesize_k3_traces.py` from real K2 marginals, see SCHEMA's trace
section for the two-sided provenance note). Specs select shapes via the
`shape:` preset key (`sweep.py SHAPE_PRESETS`; `apply_shape` hard-guards
fields and trace model). **`shape: qwen3` is the fast 4n-validation lane**
(K3 topk-16 incidence saturates at 4 nodes; Qwen3 topk-8 does not): the
AUTHENTIC Qwen3-235B-A22B shape `G=128, topk 8, H=4096, ffn 1536` on
captured Qwen3 routing — note **ffn 1536 is a 2026-08-20 correction**;
pre-correction Qwen3-shape capsules ran ffn 4096 (unrecorded — no
